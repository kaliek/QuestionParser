...
def compare_model():
    wh_data = pandas.read_csv(os.path.join(CORPUS_DIR, 'all_raw_1.csv'))
...
def build_data():
    ...
    pos_dep = ["Head_POS", "Neck_Label", "Root_POS"]
    entity = ["PER", "LOC", "OBJ", "TEM", "NUM"]
    original.extend(pos_dep)
    original.extend(entity)
    header = original
    with open(os.path.join(CORPUS_DIR, 'all_raw.csv'), "r") as train, open(os.path.join(CORPUS_DIR, 'all_raw_1.csv'), "w") as feature:
        ...
        for line in reader:
            ...
            result = [question, label, qpp.get_head(),
                qpp.get_head_pos(), qpp.get_neck_label(), qpp.get_root_pos(),
